...
#define SDA_OUT()   GPIOB_PDDR |= (1 << 0)
#define SDA_IN()    {GPIOB_PDDR &= ~(1 << 0);GPIOB_PIDR &=~ (1<<0);}
#define SCL_OUT()   GPIOB_PDDR |= (1 << 1)
#define SCL_IN()    {GPIOB_PDDR &= ~(1 << 1);GPIOB_PIDR &=~ (1<<1);}
// SDA on port B, bit 14
#define SDA_LOW()	GPIOB_PCOR |= (1 << 0) // set SDA to low
#define SDA_OPEN()  GPIOB_PSOR |= (1 << 0) // set SDA to open-drain
#define SDA_READ	(GPIOB_PDIR & GPIO_PDIR_PDI(1<<0))	// read SDA

// SCL on port B, bit 13	/* -- adapt the defines for your uC -- */
#define SCL_LOW()	GPIOB_PCOR |= (1 << 1) // set SCL to low
#define SCL_OPEN()  GPIOB_PSOR |= (1 << 0) // set SCL to open-drain
#define SCL_READ	(GPIOB_PDIR & GPIO_PDIR_PDI(1<<1))	// read SCL
